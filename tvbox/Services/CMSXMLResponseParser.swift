...
final class CMSXMLResponseParser: NSObject, XMLParserDelegate {
    ...
    static func parse(_ data: Data, sourceKey: String) throws -> (sorts: [MovieSort.SortData], homeVideos: [Movie.Video]) {
        ...
        return (delegate.sorts, delegate.videos)
    }

    func parser(_ parser: XMLParser, didStartElement elementName: String, namespaceURI: String?, qualifiedName qName: String?, attributes attributeDict: [String: String]) {
        elements.append((elementName, ""))
        if elementName == "video" { videoFields = [:] }
